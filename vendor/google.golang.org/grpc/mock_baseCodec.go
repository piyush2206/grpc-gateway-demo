// Code generated by mockery v1.0.0. DO NOT EDIT.
package grpc

import mock "github.com/stretchr/testify/mock"

// mockBaseCodec is an autogenerated mock type for the baseCodec type
type mockBaseCodec struct {
	mock.Mock
}

// Marshal provides a mock function with given fields: v
func (_m *mockBaseCodec) Marshal(v interface{}) ([]byte, error) {
	ret := _m.Called(v)

	var r0 []byte
	if rf, ok := ret.Get(0).(func(interface{}) []byte); ok {
		r0 = rf(v)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]byte)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(interface{}) error); ok {
		r1 = rf(v)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Unmarshal provides a mock function with given fields: data, v
func (_m *mockBaseCodec) Unmarshal(data []byte, v interface{}) error {
	ret := _m.Called(data, v)

	var r0 error
	if rf, ok := ret.Get(0).(func([]byte, interface{}) error); ok {
		r0 = rf(data, v)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}
